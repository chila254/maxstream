...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  /// The id of the player backing this controller, used to address the native
  /// player directly (see `AndroidVideoPlayer.reloadMedia`). -1 while unset.
  /// Vendor patch: exposed publicly (upstream marks it @visibleForTesting).
  ...
  Future<void> initialize() async {
    ...
    final creationOptions = platform_interface.VideoCreationOptions(
      ...
      viewType: viewType,
      // Vendor patch: pass the platform options (back/max buffer durations,
      // etc.) through to createWithOptions. Upstream drops them here, so the
      // Android ExoPlayer plugin never saw them.
